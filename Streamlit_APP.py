...
# Load the JSON file with a valid file path
with open('response.json', 'r') as file:  # Replace 'response.json' with the correct path to your JSON file
...
if button and uploaded_file is not None and mcq_count and subject and tone:
    with st.spinner("Loading..."):
        try:
            ...
            # Logging tokens and costs
            print(f"Total Tokens: {cb.total_tokens}")
            print(f"Prompt Tokens: {cb.prompt_tokens}")
            print(f"Completion Tokens: {cb.completion_tokens}")
            print(f"Total Cost: {cb.total_cost}")
            
            # Check if response is a dictionary and extract quiz
